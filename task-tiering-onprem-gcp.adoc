...
* Un cluster ONTAP on-premise che esegue ONTAP 9.6 o versione successiva e dispone di una connessione HTTPS allo storage cloud Google. https://docs.netapp.com/us-en/cloud-manager-ontap-onprem/task-discovering-ontap.html["Scopri come individuare un cluster"^].
...
* https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-quick-start-connector-google.html["Implementazione di un connettore in GCP"^]
...
+
** Una connessione HTTPS tramite la porta 443 al servizio di tiering BlueXP e allo storage Google Cloud (https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-set-up-networking-google.html#endpoints-contacted-for-day-to-day-operations["vedere l'elenco degli endpoint"^])
...
NOTE: Se si prevede di configurare il tiering BlueXP per utilizzare classi di storage a basso costo in cui i dati a più livelli passeranno dopo un certo numero di giorni, non è necessario selezionare alcuna regola del ciclo di vita durante la configurazione del bucket nell'account GCP. BlueXP Tiering gestisce le transizioni del ciclo di vita.
...
.. *Storage Class Life Cycle*: Il tiering BlueXP gestisce le transizioni del ciclo di vita dei dati a più livelli. I dati iniziano nella classe _Standard_, ma è possibile creare regole per spostare i dati in altre classi dopo un certo numero di giorni.
+
Selezionare la classe di storage Google Cloud a cui si desidera trasferire i dati a più livelli e il numero di giorni prima che i dati vengano spostati, quindi fare clic su *continua*. Ad esempio, la schermata seguente mostra che i dati a più livelli vengono spostati dalla classe _Standard_ alla classe _Nearline_ dopo 30 giorni di storage a oggetti, quindi alla classe _Coldline_ dopo 60 giorni di storage a oggetti.
...
+
image:screenshot_tiering_lifecycle_selection_gcp.png["Una schermata che mostra come selezionare ulteriori classi di storage in cui i dati vengono spostati dopo un certo numero di giorni."]
