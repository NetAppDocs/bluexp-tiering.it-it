...
* Puoi scegliere la classe di storage/il Tier di accesso del cloud provider predefinito oppure utilizzare la gestione del ciclo di vita per spostare i dati più vecchi su più livelli a un livello più conveniente
...
TIP: I sistemi Cloud Volumes ONTAP sono di sola lettura grazie al tiering BlueXP. https://docs.netapp.com/us-en/cloud-manager-cloud-volumes-ontap/task-tiering.html["È possibile impostare il tiering per i sistemi Cloud Volumes ONTAP dall'ambiente di lavoro in BlueXP"^].
...
. Se è stata applicata una regola del ciclo di vita ai dati a più livelli (disponibile solo per alcuni provider), i dati a più livelli meno recenti vengono spostati in un livello più conveniente dopo un certo numero di giorni.
...
* Il backup e il ripristino di BlueXP sono influenzati in modo analogo dai volumi impostati con un criterio di tiering. https://docs.netapp.com/us-en/cloud-manager-backup-restore/concept-ontap-backup-to-cloud.html#fabricpool-tiering-policy-considerations["Vedi considerazioni sui criteri di tiering con il backup e ripristino di BlueXP"^].
